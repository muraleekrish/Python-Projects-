...
def parse(file_name):
 try:
    ...
    return response_dict
        ...
        sendEmail("muralee@hayagriv.com","hayagriventerprises@gmail.com",ErrorFilePath)
...
def HtmlFormat(response_dict):       
 ...
 # print response_dict 
   ...
   f = open(HTMLFILE, 'w')
   t = HTML.Table(header_row=['URL(Request Type)','Max Time','Min Time', 'Avg ResponseTime(ms)','Number of Hits'],
     col_width=['80%','10%','10%','10%','10%'],
     col_align=['left','center','center','center','Center'],
     col_styles=['font-size: 105%','font-size: 105%','font-size: 105%','background-color:lightgrey','font-size:105%'])
   for k, v in response_dict.items():
       sum_response_times = 0.0
       maximum = max(v)
       minimum = min(v)
...
if __name__ == "__main__":
 ...
 ResponseFilePath = 'C:\Documents and Settings\murali\pyth\HTML\ResponseTime-Report.html'
 sendEmail("muralee@hayagriv.com","hayagriventerprises@gmail.com",ResponseFilePath)
